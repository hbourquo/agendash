...
module.exports = function (agenda, options) {
  ...
  const getJobs = function (job, state, options) {
    ...
    if (options.query && options.property) {
      if (options.isObjectId) {
        // preMatch[options.property] = ObjectId(options.query); => TypeError: Class constructor ObjectId cannot be invoked without 'new'
  ...
  const requeueJobs = async (jobIds) => {
    ...
    const jobs = await collection
      .find({
        // _id: { $in: jobIds.map((jobId) => ObjectId(jobId)) },  => TypeError: Class constructor ObjectId cannot be invoked without 'new'
